import { MapPin, Users, Briefcase as BriefcaseIcon } from "lucide-react";
...
export function JobsList({ jobs, applicantCounts, canEdit, canDelete, onUpdateStatus, onViewCandidates }: JobsListProps) {
  if (jobs.length === 0) {
    return (
      ...
      {jobs.map((jobPosting) => (
        <div key={jobPosting.id} className="flex flex-col gap-3 rounded-xl border border-border bg-surface-card p-4">
          ...
          <div className="flex gap-2">
            ...
            {canEdit && jobPosting.status !== "Closed" && (
              <Button
                variant="secondary"
                size="sm"
            ...
            {canDelete && jobPosting.status !== "Closed" && (
